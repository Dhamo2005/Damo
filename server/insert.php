...
if (empty($name && $email && $phone_number && $class && $medium && $about && $place && $type && $password && $confirm_password)) {
    ...
    if ($statement->rowCount() == 0) {
        ...
        $statement = DB::con()->prepare("INSERT INTO `users` (`name`, `phone_number`, `class`, `medium`, `email`, `password`, `about`, `Joined`, `place`, `type`,`gender`) VALUES (:name, :phone_number, :class, :medium, :email,:password,:about, now(), :place,:type,:gender)");

        if ($statement->execute(['name' => $name, 'phone_number' => $phone_number, 'class' => $class, 'medium' => $medium, 'email' => $email, 'password' => $password_hash, 'about' => $about, 'place' => $place, 'type' => $type, 'gender' => $gender])) {
            echo 1;
        }else{
            echo 0;
        }
